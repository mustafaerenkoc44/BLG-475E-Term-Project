...
loop in Phase~1: we did not accept generations that looked plausible;
rather, we accepted them only after they compiled and passed the dataset
base tests.
...
rather than a mere text-generation task. The design of our Phase~1
...
engineering. Our findings support both views; the two local coder models
achieved complete base-test success only after normalisation and
targeted repair on seven tasks.
...
towards uncovered paths. As part of our improved-test step we utilised
black-box reasoning combined with branch-coverage information to
develop targeted tests for missed or poorly exercised behaviour.
