...
- **Task:** `Java/21` (folder `Java_021_rescaleToUnit`)
- **Step:** `test-improvement`
- **Base artifact:** `tasks/Java_021_rescaleToUnit/generated-tests/improved/DeepSeek-Coder-1.3B-Instruct-GGUF/DatasetImprovedTest.java`
...
Target: rescaleToUnit in tasks/Java_021_rescaleToUnit/generated-code/DeepSeek-Coder-1.3B-Instruct-GGUF/Solution.java.
...
| V2 | Unsorted list with negative and positive values. | `numbers=[-2.0, 3.0, 1.0]` | Yes (improved) | Checks min and max discovery. |
| V3 | List with repeated interior values. | `numbers=[1.0, 1.0, 5.0, 3.0]` | Yes (improved) | Interior duplicates should map consistently. |

## Invalid Classes

| Class ID | Description | Representative Input | Covered By Existing Test? | Notes |
|---|---|---|---|---|
| I1 | All values are equal. | `numbers=[2.0, 2.0, 2.0]` | Yes (improved) | Would force division by zero in the usual formula. |
| I2 | Fewer than two elements. | `numbers=[4.0]` | Yes (improved) | Outside the prompt contract. |

## Boundary Conditions

| Class ID | Description | Representative Input | Covered By Existing Test? | Notes |
|---|---|---|---|---|
| B1 | Exactly two elements. | `numbers=[2.0, 6.0]` | Yes (improved) | Smallest valid rescaling case. |
| B2 | Input already spans 0 and 1. | `numbers=[0.0, 0.5, 1.0]` | Yes (improved) | Should remain unchanged. |
...
at logs/DeepSeek-Coder-1.3B-Instruct-GGUF/Java_21_test-improvement.raw.txt when available;
...
- `improvedSpansNegativeAndPositive_V2` - covers an equivalence / boundary class flagged as 'No' or 'Partially' in equivalence_classes.md
- `improvedRepeatedInteriorMapsConsistently_V3` - covers an equivalence / boundary class flagged as 'No' or 'Partially' in equivalence_classes.md
- `improvedInputIsMutatedInPlace_B1` - covers an equivalence / boundary class flagged as 'No' or 'Partially' in equivalence_classes.md
- `improvedMutationAllEqualTriggersNaN_I1` - mutation-based guard (see docs/analysis/mutation_testing_strategy.md)
...
compile_success=true and junit_success=true for (Java_021_rescaleToUnit, DeepSeek-Coder-1.3B-Instruct-GGUF).
