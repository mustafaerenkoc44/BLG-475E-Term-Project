...
class DatasetImprovedTest {
    ...
    void improvedMutationBelowTwoReturnsEmptyList_I1_I2() {
        ...
        Assertions.assertEquals(
                List.of(),
                s.factorize(1),
                "Invalid class I1 (mutation): DeepSeek's `i <= n` loop with n=1 never executes the body");
        Assertions.assertEquals(
                List.of(),
                s.factorize(0),
                "Invalid class I2 (mutation): n=0 keeps the loop from entering so no factors are emitted");
    }
}
