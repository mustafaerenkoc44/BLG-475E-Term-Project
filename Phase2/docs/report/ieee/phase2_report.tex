...
%  Phase 2 Report (IEEE conference style)
...
multiple test cases (Qwen $7/15$, DeepSeek $9/15$); the edited combined
prompt makes token boundaries, line-number semantics, whole-word matching
and invalid-input handling explicit and lifts both models to the full
$15/15$ suite. The selected final implementation passes all $15$ JUnit
tests, reaches $70/72 = 97.22\,\%$ branch coverage and $109/110 = 99.09\,\%$
line coverage with JaCoCo, and keeps every Phase~1 mutation-based
guardrail inherited from tasks \texttt{Java/18}, \texttt{Java/23} and
\texttt{Java/27}. Our results indicate that at the integration level the
prompt specification matters at least as much as the choice of underlying
model.
\end{abstract}

\begin{IEEEkeywords}
Large language models, integration testing, JUnit, JaCoCo, branch coverage,
prompt engineering, HumanEval-X, software quality.
\end{IEEEkeywords}
...
withdrew early in the semester. Phase~2 preserves the Phase~1 workload
balance (approximately $65/35$) and the same two models,
...
The Phase~2 JUnit suite contains $11$ integration tests and $4$ focused
regression tests, for a total of $15$ tests shared by all five
...
the edited prompt restores full pass rates for both models; the selected
final implementation slightly improves branch coverage again.
...
\midrule
original-combined & Qwen     & 7 / 15  & n/a \\
original-combined & DeepSeek & 9 / 15  & n/a \\
edited-combined   & Qwen     & 15 / 15 & 66 / 68 = 97.06\,\% \\
edited-combined   & DeepSeek & 15 / 15 & 61 / 64 = 95.31\,\% \\
selected-final    & manual   & 15 / 15 & 70 / 72 = 97.22\,\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Qwen     & zero-based / non-unique line numbers; raw space-splitting destroys \texttt{can't}/\texttt{co-op}; case-sensitive substring search; no canonicalization for uppercase or alphanumeric queries \\
DeepSeek & punctuation stripping changes token identity; substring matching inside longer words; over-counting in case-insensitive search; weak handling of punctuation around trimmed queries; alphanumeric over-counting \\
\bottomrule
\end{tabular}
\end{table}
...
Both edited-prompt variants reach $15/15$ JUnit success and high branch
coverage (Table~\ref{tab:compare}). The edited prompt therefore closes
\emph{more} defects than switching between the two models: in our
experiment prompt refinement dominates model selection. This is consistent
with the recent literature on LLM unit-test generation, where explicit
specifications have been repeatedly shown to outperform model
upsizing~\cite{metatestgen,yang2024,panta2024,lit_review}.
...
\begin{itemize}
  \item branch coverage $70 / 72 = 97.22\,\%$,
  \item line coverage $109 / 110 = 99.09\,\%$,
  \item cyclomatic complexity covered $49 / 51 = 96.08\,\%$.
\end{itemize}

Only two branches remain uncovered (Section~\ref{sec:residual}).

\subsection{Residual Uncovered Branches}\label{sec:residual}

\begin{itemize}
  \item \texttt{canonicalizeWord}: an \texttt{isEmpty} guard on a trimmed
    token. Tokenization itself already rejects blank tokens, so the guard
    cannot be reached through the public API.
  \item \texttt{tokenizeWords}: a \texttt{line == null} guard. All public
    entry points split concrete (non-null) strings before calling this
    helper.
\end{itemize}

Both residual misses are intentional defensive branches, documented in
\texttt{Phase2/docs/analysis/black\_box\_assessment.md} and in
\texttt{final\_coverage\_summary.md}. Removing them would weaken the class
on future refactors (e.g.\ reusing the helpers from another class) for no
real coverage gain.
...
four Phase~2 regression tests in Table~\ref{tab:reg-tests}. In other
words, the Phase~1 mutation-based guardrails continue to protect Phase~2
at the helper level, even though Phase~2 focuses on integration-level
behaviour.
...
\texttt{BookScan.class}, re-runs the $15$ integration/regression tests
...
$15/15$ suite, reaches $97.22\,\%$ branch coverage under both the
prompt-comparison harness and the Maven build, and preserves all Phase~1
helper-level mutation guardrails. The repository is therefore ready for
submission and, crucially, the Phase~2 results strengthen rather than
contradict the Phase~1 headline: \emph{once the guardrails are in place,
prompt engineering does more work than model selection}.

% ---------------------------------------------------------------------------
\section*{Acknowledgement}

The source repository, including the full prompt-comparison harness,
Maven build, GitHub Actions workflow and raw JaCoCo artefacts, is
available at
\url{https://github.com/mustafaerenkoc44/BLG-475E-Term-Project}. Group
workload:
\textbf{Mustafa Eren KO\c{C}} (150190805, \(\approx 65\,\%\)):
\texttt{BookScan} design, prompt-comparison harness, Maven configuration,
integration-test automation, coverage analysis, result aggregation, CI
workflow, Git history management;
\textbf{Onat Bar{\i}\c{s} ERCAN} (150210075, \(\approx 35\,\%\)):
prompt review, black-box assessment support, failure classification,
report refinement, IEEE LaTeX formatting, final submission
cross-checks.

% ---------------------------------------------------------------------------
\bibliographystyle{IEEEtran}
\bibliography{phase2_report}
